// ============================================================
// ENDevo Q12 Gap Analysis — Deterministic Engine
// Spec locked by Niki (PO), April 2026
// All plan content is deterministic. Claude is only used for
// personalization wrapper copy (see JesseWrapper interface).
// ============================================================
...
// ============================================================
// ACTION POOL — 2 per domain + Day 7 consolidation
// Copy is LOCKED per Niki PO spec. Do not modify.
// ============================================================
...
export const ACTION_POOL: Action[] = [
  {
    ...
    howTo: "iPhone: Settings → Apple ID → Legacy Contact. Android: Google Inactive Account Manager. Add primary + secondary + tertiary.",
  },
  {
    id: "DIGITAL_B",
    domain: "Digital",
    slot: "B",
    title: "Install a password manager and add your top 10 accounts",
    time: "15 min",
    socialProof: "Only 22% of adults use a password manager. A locked password vault dies with you without emergency access.",
    howTo: "Download 1Password, Bitwarden, or Keeper (free tier works). Import your 10 most critical logins. Configure emergency access for your Digital Executor.",
  },
  {
    id: "LEGAL_A",
    domain: "Legal",
    slot: "A",
    title: "Identify Primary, Secondary, Tertiary for Executor — and ask the primary",
    time: "15 min",
  ...
  },
  {
    ...
    domain: "Legal",
    slot: "B",
    title: "Status-check your Will: exists / outdated / does not exist",
    time: "15 min",
    socialProof: "76% of people die without a current Will. State intestacy law then decides who gets what, rarely matching what you wanted.",
    howTo: "One of three: (1) if you have one, text your Executor the location. (2) If outdated, book an attorney consult this week. (3) If none, write ONE sentence: 'If I died tomorrow, I would want ___.'",
  },
  {
    id: "FINANCIAL_A",
    domain: "Financial",
    ...
    time: "10 min",
    socialProof: "Outdated beneficiaries are the #1 estate planning mistake. They override the Will entirely. Ex-spouses regularly inherit by default.",
  ...
  },
  {
    ...
    domain: "Financial",
    slot: "B",
    title: "List your 5 most important financial accounts (institution + last 4)",
    time: "15 min",
    socialProof: "67% of families struggle to find accounts after death. $58B sits unclaimed in the US right now.",
    howTo: "Open a password-protected doc. Write: institution name, account type, last 4 digits, beneficiary. 5 accounts. Store in your password manager.",
  },
  {
    id: "PHYSICAL_A",
    domain: "Physical",
    slot: "A",
    title: "Decide burial vs cremation. Write it. Name the funeral home.",
    time: "10 min",
  ...
  },
  {
    ...
    domain: "Physical",
    slot: "B",
    title: "Draft your advance directive preference in 3 sentences",
    time: "15 min",
    socialProof: "77% don't have a Medical Advance Directive. Hospitals default to maximum intervention without it.",
...
// Tie-break order per Niki PO spec: Digital → Legal → Financial → Physical
// "This ensures users always start with a fast digital win regardless of ties."
...
export function runAssessment(name: string, answers: Answer[]): AssessmentResult {
  ...
  return { name, totalScore, percentReady, band, domainResults, plan };
}
...
// ============================================================
// CLAUDE WRAPPER STUB — swap for live API call after Niki sign-off
// ============================================================
...
export function getStaticWrapper(result: AssessmentResult): JesseWrapper {
  const weakest = result.domainResults[0].domain;
  const copy: Record<Band, { opening: string; closing: string }> = {
    "AT RISK": {
      opening: `Hi ${result.name}. Your score is ${result.percentReady}% ready. That tells me you have real gaps across multiple domains, and that is okay. Most people are here. Your plan starts with ${weakest}, your biggest gap, because closing it first changes everything else. Tomorrow is Day 1.`,
      closing: `${result.name}, seven days ago you were at ${result.percentReady}%. You have now done what 96% of people never will. Your next step: a 1:1 with Niki to lock in what you have started.`,
    },
    "SOMEWHAT PREPARED": {
      opening: `Hi ${result.name}. Your score is ${result.percentReady}% ready, partial coverage with clear gaps. Your plan opens with ${weakest} because closing that one gap creates the most protection. Tomorrow is Day 1.`,
      closing: `${result.name}, you came in at ${result.percentReady}% and spent seven days closing real gaps. The next move is either a 1:1 with Niki or the 7-Week Sprint to go deeper.`,
    },
    PREPARED: {
      opening: `Hi ${result.name}. Your score is ${result.percentReady}% ready, a strong foundation. Your plan still opens with ${weakest}, the one area with room to grow. Tomorrow is Day 1.`,
      closing: `${result.name}, you started at ${result.percentReady}% and used this week to fine-tune. The 7-Week Sprint is built for people at your level who want to go from prepared to bulletproof.`,
    },
  };
  return {
    opening: copy[result.band].opening,
    day1Note: "Day 1 is done. Tomorrow we move to your second weakest domain.",
    day2Note: "Two days in. Tomorrow is the halfway lean-in.",
    day3Note: "Halfway there. Tomorrow you touch your fourth and final domain.",
    day4Note: "Every domain has now been touched. Tomorrow we go back to your weakest for a second action.",
    day5Note: "Back to the hardest domain with a deeper move. One day left before consolidation.",
    day6Note: "Six days down. Tomorrow is consolidation and your next step.",
    day7Closing: copy[result.band].closing,
  };
}
